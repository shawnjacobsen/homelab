...
>
> 1. **Host creates the NVIDIA device nodes at boot.** `nvidia-uvm` (and `nvidiactl`/`modeset`) are created *lazily on first driver/CUDA use*, so after a cold boot they don't exist yet — a bind-mount of a missing node is skipped and the containers fail to start (exit 128). A host systemd oneshot **`nvidia-uvm-init.service`** (ordered `Before=pve-guests.service`) forces the full node set to exist before LXC 101 starts, via `nvidia-modprobe -c 0 -u` + `nvidia-modprobe -m` + `nvidia-smi`. Creating only `uvm` here is not enough — the other nodes must also exist before the LXC starts, or a stack loses the boot race. *(Host-only, not version-controlled.)*
